...
log_info()  { echo -e "${GREEN}[crop-venv]${NC} $1"; }
log_warn()  { echo -e "${YELLOW}[crop-venv]${NC} $1"; }
log_error() { echo -e "${RED}[crop-venv]${NC} $1"; }
...
if [ -f "$VENV_DIR/bin/python3" ] || [ -f "$VENV_DIR/Scripts/python.exe" ]; then
  ...
  if "$VENV_PYTHON" -c "import cv2; import numpy; import mediapipe; from PIL import Image" 2>/dev/null; then
